...
public abstract class TBRecyclerAdapter <T, V extends TBViewHolder> extends Adapter<V> {
    //load more
    private boolean mNextLoadEnable = false;
    private boolean mLoadMoreEnable = false;
    private boolean mLoading = false;
    private TBLoadMoreView mLoadMoreView = new TBSimpleLoadMoreView();
    ...
    protected static final String TAG = TBRecyclerAdapter.class.getSimpleName();
     ...
     */
    public void setLoadMoreView(TBLoadMoreView loadingView) {
    ...
    public void loadMoreEnd(boolean gone) {
        ...
        if (gone) {
            ...
            mLoadMoreView.setLoadMoreStatus(TBLoadMoreView.STATUS_END);
    ...
    public void loadMoreComplete() {
        ...
        mLoading = false;
        mNextLoadEnable = true;
        mLoadMoreView.setLoadMoreStatus(TBLoadMoreView.STATUS_DEFAULT);
        notifyItemChanged(getLoadMoreViewPosition());
    }
    ...
    public void loadMoreFail() {
        ...
        mLoading = false;
        mLoadMoreView.setLoadMoreStatus(TBLoadMoreView.STATUS_FAIL);
        notifyItemChanged(getLoadMoreViewPosition());
    }
     ...
     */
    public TBRecyclerAdapter(Context context, @LayoutRes int layoutResId, @Nullable List<T> data) {
    ...
    public TBRecyclerAdapter(Context context, @Nullable List<T> data) {
        this(context, -1, data);
    }

    public TBRecyclerAdapter(Context context, @LayoutRes int layoutResId) {
    ...
    //     mData.add(data);
    //     notifyItemInserted(getDataSize() + getHeaderLayoutCount());
    //     compatibilityDataSizeChanged(1);
    // }
    ...
    //     notifyItemRangeChanged(internalPosition, getDataSize() - internalPosition);
    // }
    ...
    public void addData(@NonNull Collection<? extends T> newData) {
        ...
        notifyItemRangeInserted(getDataSize() - newData.size() + getHeaderLayoutCount(), newData.size());
        compatibilityDataSizeChanged(newData.size());
    }
    ...
    private void compatibilityDataSizeChanged(int size) {
        final int dataSize = mData == null ? 0 : getDataSize();
        if (dataSize == size) {
            notifyDataSetChanged();
        }
    }
    ...
    private V getLoadingView(ViewGroup parent) {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mLoadMoreView.getLoadMoreStatus() == TBLoadMoreView.STATUS_FAIL) {
                    notifyLoadMoreToLoading();
                }
                if (mEnableLoadMoreEndClick && mLoadMoreView.getLoadMoreStatus() == TBLoadMoreView.STATUS_END) {
                    notifyLoadMoreToLoading();
                }
            }
        });
    ...
    public void notifyLoadMoreToLoading() {
        if (mLoadMoreView.getLoadMoreStatus() == TBLoadMoreView.STATUS_LOADING) {
            return;
        }
        mLoadMoreView.setLoadMoreStatus(TBLoadMoreView.STATUS_DEFAULT);
        notifyItemChanged(getLoadMoreViewPosition());
            ...
            gridManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
                ...
                public int getSpanSize(int position) {
                    ...
                    if (mSpanSizeLookup == null) {
                        ...
                        return (isFixedViewType(type)) ? gridManager.getSpanCount() : mSpanSizeLookup.getSpanSize(
                            gridManager,
    ...
    private void bindViewClickListener(final TBViewHolder baseViewHolder) {
        ...
        if (getOnItemClickListener2() != null) {
            view.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    //setOnItemClick(v, baseViewHolder.getLayoutPosition() - getHeaderLayoutCount());
                    int position = baseViewHolder.getLayoutPosition() - getHeaderLayoutCount();
                    getOnItemClickListener2().onItemClick(TBRecyclerAdapter.this, view, position,
    ...
    public void setOnItemClick(View v, int position) {
        getOnItemClickListener().onItemClick(TBRecyclerAdapter.this, v, position);
    }
    ...
    public boolean setOnItemLongClick(View v, int position) {
        return getOnItemLongClickListener().onItemLongClick(TBRecyclerAdapter.this, v, position);
    }
    ...
    protected V createBaseViewHolder(View view) {
        ...
        // 泛型擦除会导致z为null
        if (z == null) {
            k = (V)new TBViewHolder(view);
        } else {
            k = createGenericKInstance(z, view);
        }
        return k != null ? k : (V)new TBViewHolder(view);
    }
        ...
        if (type instanceof ParameterizedType) {
            ...
            for (Type temp : types) {
                if (temp instanceof Class) {
                    ...
                    if (TBViewHolder.class.isAssignableFrom(tempClass)) {
                        return tempClass;
                    }
                } else if (temp instanceof ParameterizedType) {
                    Type rawType = ((ParameterizedType)temp).getRawType();
                    if (rawType instanceof Class && TBViewHolder.class.isAssignableFrom((Class<?>)rawType)) {
    ...
    /**
     * When the current adapter is empty, the TBRecyclerAdapter can display a special view
    ...
    public View getEmptyView() {
        if (mEmptyLayout != null && mEmptyLayout.getChildCount() != 0) {
            return mEmptyLayout.getChildAt(0);
        }
        return null;
    }
    ...
    private void autoLoadMore(int position) {
        ...
        if (mLoadMoreView.getLoadMoreStatus() != TBLoadMoreView.STATUS_DEFAULT) {
            return;
        }
        mLoadMoreView.setLoadMoreStatus(TBLoadMoreView.STATUS_LOADING);
        if (!mLoading) {
            mLoading = true;
            if (getRecyclerView() != null) {
    ...
    public View getViewByPosition(RecyclerView recyclerView, int position, @IdRes int viewId) {
        ...
        TBViewHolder viewHolder = (TBViewHolder)recyclerView.findViewHolderForLayoutPosition(position);
        if (viewHolder == null) {
            return null;
        }
    ...
    public interface OnItemChildClickListener {
         ...
         */
        void onItemChildClick(TBRecyclerAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemChildLongClickListener {
         ...
         */
        boolean onItemChildLongClick(TBRecyclerAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemLongClickListener {
         ...
         * @return true if the callback consumed the long click ,false otherwise
         */
        boolean onItemLongClick(TBRecyclerAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemClickListener {
         ...
         */
        void onItemClick(TBRecyclerAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemClickListener2 <T> {
        ...
        void onItemClick(TBRecyclerAdapter adapter, View view, int position, T model);
    }
    ...
    public void setOnItemClickListener2(@Nullable OnItemClickListener2<T> listener) {
